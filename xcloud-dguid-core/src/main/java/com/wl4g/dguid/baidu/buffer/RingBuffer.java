 ...
 */
package com.wl4g.dguid.baidu.buffer;
...
import com.wl4g.dguid.baidu.utils.PaddedAtomicLong;
...
public class RingBuffer {
	 ...
	 *            must be positive & a power of 2
	 * @param paddingFactor
	 *            percent in (0 - 100). When the count of rest available UIDs
	 *            reach the threshold, it will trigger padding buffer<br>
	 *            Sample: paddingFactor=20, bufferSize=1000 -> threshold=1000 *
	 *            20 /100, padding buffer will be triggered when
	 *            tail-cursor<threshold
	 */
	public RingBuffer(int bufferSize, int paddingFactor) {
		// check buffer size is positive & a power of 2; padding factor in (0,
		// 100)
	 ...
	 * We use 'synchronized' to guarantee the UID fill in slot & publish new
	 * tail sequence as atomic operations<br>
	 * 
	 * <b>Note that: </b> It is recommended to put UID in a serialize way, cause
	 * we once batch generate a series UIDs and put the one by one into the
	 * buffer, so it is unnecessary put in multi-threads
	 *
	 * @param uid
	...
	public synchronized boolean put(long uid) {
		...
		// another word,
		// the take operation can't consume the UID we just put, until the tail
		// is published(tail.incrementAndGet())
		return true;
	}
	...
	/**
	 * Take an UID of the ring at the next cursor, this is a lock free operation
	 * by using atomic cursor
	 * <p>
	 * 
	 * Before getting the UID, we also check whether reach the padding
	 * threshold, the padding buffer operation will be triggered in another
	 * thread<br>
	...
	public long take() {
		...
		if (currentTail - nextCursor < paddingThreshold) {
			LOGGER.info("Reach the padding threshold:{}. tail:{}, cursor:{}, rest:{}", paddingThreshold, currentTail, nextCursor,
					currentTail - nextCursor);
			bufferPaddingExecutor.asyncPadding();
		}

		// cursor catch the tail, means that there is no more available UID to
		// take
		...
		// Note that: Step 2,3 can not swap. If we set flag before get value of
		// slot, the producer may overwrite the
	...
	/**
	 * Policy for {@link RejectedTakeBufferHandler}, throws
	 * {@link RuntimeException} after logging
	 */
